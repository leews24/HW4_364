...
# DONE 364: All templates you need are provided and should not be edited. However, you will need to inspect the templates that exist in order to make sure you send them the right data!
...
app.config["SQLALCHEMY_DATABASE_URI"] = os.environ.get('DATABASE_URL') or "postgres://postgres:0000@localhost/wwsleeHW4db" # DONE 364: You should edit this to correspond to the database name YOURUNIQNAMEHW4db and create the database of that name (with whatever your uniqname is; for example, my database would be jczettaHW4db). You may also need to edit the database URL further if your computer requires a password for you to run this.
...
user_collection = db.Table('user_collection', db.Column('gifs_id', db.Integer, db.ForeignKey('gifs.id')), db.Column('personalgifs.id', db.Integer, db.ForeignKey('personalgifs.id')))
...
class User(UserMixin, db.Model):
    ...
    #DONE 364: In order to complete a relationship with a table that is detailed below (a one-to-many relationship for users and gif collections), you'll need to add a field to this User model. (Check out the TODOs for models below for more!)
...
# DONE 364: Read through all the models tasks before beginning them so you have an understanding of what the database structure should be like. Consider thinking about it as a whole and drawing it out before you write this code.
...
def get_gifs_from_giphy(search_string):
    ...
    for gif in results['data']:
        dict = {}
        dict['title'] = gif['title']
        dict['url'] = gif['embed_url']
        ret.append(dict)
    return ret
...
def get_or_create_gif(title, url):
    ...
    gifcheck = Gif.query.filter_by(title=title).first()
    if gifcheck:
        return gifcheck
    else:
...
def get_or_create_search_term(term):
    ...
    searchcheck = SearchTerm.query.filter_by(term=term).first()
    ...
    # This function should invoke the get_gifs_from_giphy function to get a list of gif data from Giphy.
        retgifs = get_gifs_from_giphy(term)
    # It should iterate over that list acquired from Giphy and invoke get_or_create_gif for each, and then append the return value from get_or_create_gif to the search term's associated gifs (remember, many-to-many relationship between search terms and gifs, allowing you to do this!).
        newsearch = SearchTerm(term=term, gifs=[])
        for gif in retgifs:
            gifitem = get_or_create_gif(gif['title'], gif['url'])
            newsearch.gifs.append(gifitem)
...
def get_or_create_collection(name, current_user, gif_list=[]):
    ...
    collectioncheck = PersonalGifCollection.query.filter_by(name=name, users=current_user.id).first()
    ...
    if collectioncheck:
        ...
        newcollection = PersonalGifCollection(name=name, users=current_user.id, gifs=[])
        for gif in gif_list:
            newcollection.gifs.append(gif)
...
def index():
    ...
    if form.validate_on_submit():
        ...
        return redirect('/gifs_searched/' + search.term)
...
def search_terms():
    ...
    searchterms = SearchTerm.query.all()
    return render_template('search_terms.html', all_terms=searchterms)
...
def create_collection():
    ...
    gifs = Gif.query.all()
    choices = [(str(g.id), str(g.title)) for g in gifs]
    ...
    if form.validate_on_submit():
        ...
        for pick in form.gif_picks.data:
            giflist.append(get_gif_by_id(pick))
...
def collections():
    # pass # Replace with code
    # DONE 364: This view function should render the collections.html template so that only the current user's personal gif collection links will render in that template. Make sure to examine the template so that you send it the correct data!
    collection = PersonalGifCollection.query.filter_by(users=current_user.id).all()
